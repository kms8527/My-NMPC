...
function lamda = Get_Lamda(L,X,beta,delta,lamda,N,Y_,P0,Q,C,i,mov_obs_num,sta_obs_num,Sj,Sl,front_car,adj_behind_car,adj_front_car,C_star,delt)
    if i==N+1
        lamda(:,N+1) = (-Y_(:,N+1)'*P0*C)';
    else
        lamda(:,i) = lamda(:,i+1)'*fx(L,X,beta,delta,i,delt)-Y_(:,i)'*Q*C; 
    end
    for k=1:mov_obs_num
...
function result=fx(L,X,beta,delta,i,delt)
    result(1,1:4)=[1; 0; delt*-X(4,i)*sin(X(3,i)+beta(i)); delt*cos(X(3,i)+beta(i))];
    result(2,1:4)=[0; 1; delt*X(4,i)*cos(X(3,i)+beta(i)); delt*sin(X(3,i)+beta(i))];
    result(3,1:4)=[0; 0; 1; delt*X(4,i)/L*cos(beta(i))*tan(delta(i))];
    result(4,1:4)=[0; 0; 0; 1];
end
